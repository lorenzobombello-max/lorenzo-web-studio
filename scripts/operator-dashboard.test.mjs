...
test("recruitment publication controller is server-confirmed and stale-safe", async () => {
  assert.deepEqual(recruitmentPublicationResponse({ enabled: true }), { enabled: true });
  assert.deepEqual(recruitmentPublicationRequest(false), { action: "set_recruitment_publication_enabled", enabled: false });
  assert.throws(()=>recruitmentPublicationResponse({ enabled: true, owner: true }), /INVALID_RECRUITMENT_PUBLICATION_RESPONSE/);
  assert.throws(()=>recruitmentPublicationRequest("false"), /INVALID_RECRUITMENT_PUBLICATION_INPUT/);
  const pending = [];
  const calls = [];
  const controller = createRecruitmentPublicationController({
    load: (request)=>{ calls.push(request); return new Promise((resolve)=>pending.push(resolve)); },
    execute: async (request)=>{ calls.push(request); return { enabled: request.enabled }; },
  });
  const stale = controller.refresh();
  const current = controller.refresh();
  ...
  assert.deepEqual(calls.at(-1), { action: "set_recruitment_publication_enabled", enabled: false });
});
...
test("recruitment owner workspace exposes bounded content and separate lifecycle dialogs", async () => {
  ...
  const publicationDialog = html.match(/<dialog id="recruitmentPublicationDialog"[\s\S]*?<\/dialog>/)?.[0] || "";
...
test("all operator dialogs use one exclusive responsive modal type authority", async () => {
  ...
  const expectedTypes = new Map([
    ...
    ["recruitmentPublicationDialog", "operator-modal--action-confirm"],
  ...
  assert.match(css, /@media \(min-width:901px\) \{[\s\S]*?--operator-brand-logo-size:clamp\(6\.25rem,calc\(4vw \+ 3rem\),14rem\)/);
  ...
  assert.match(css, /dialog\.dossiers-command-dialog \{ width:68vw; overflow:auto; \}/);
...
test("operator dashboard assets use explicit Pages-compatible release identities", async () => {
  ...
  assert.deepEqual([cssUrl, guardUrl, dashboardUrl], [
    "/assets/css/operator-dashboard.css?v=20260922-action-message-readable-r2",
    "/assets/js/operator-dashboard-guard.mjs?v=20260920-launch-recovery-r1",
    "./operator-dashboard.js?v=20260917-pre-project-workspace-r2",
  ]);
  for (const [url, release] of [[cssUrl, "20260922-action-message-readable-r2"], [guardUrl, "20260920-launch-recovery-r1"], [dashboardUrl, "20260917-pre-project-workspace-r2"]]) {
...
test("Finance expense modals share the fixed official company branding", async () => {
  ...
  assert.match(css, /--operator-brand-logo-size:clamp\(6\.25rem,calc\(4vw \+ 3rem\),14rem\)/);
  assert.match(css, /--operator-brand-wordmark-size:clamp\(1\.7rem,calc\(\.5vw \+ 1\.25rem\),3rem\)/);
  assert.match(css, /--operator-brand-block-size:clamp\(8\.25rem,calc\(4vw \+ 5rem\),16rem\)/);
  assert.match(css, /\.finance-modal-brand img \{ width:var\(--operator-brand-logo-size\); height:var\(--operator-brand-logo-size\); \}/);
  const ultrawideAuthority = css.match(/@media \(min-width:2000px\) \{([\s\S]*?)\n\}/)?.[1] || "";
  assert.doesNotMatch(ultrawideAuthority, /finance-modal-brand/);
  const logoAt = (viewport) => Math.min(224, Math.max(100, .04 * viewport + 48));
  const modalAt = (viewport) => .9 * viewport;
  const targetRatios = new Map([[1365, [7.5, 8.5]], [1920, [7, 8]], [2278, [6.5, 7.5]], [2560, [6.5, 7.5]], [3072, [6, 7]], [3840, [5.5, 6.5]]]);
  const logoSizes = [...targetRatios.keys()].map(logoAt);
  assert.ok(logoSizes.every((size, index) => index === 0 || size > logoSizes[index - 1]));
  for (const [viewport, [minimum, maximum]] of targetRatios) {
    const ratio = logoAt(viewport) / modalAt(viewport) * 100;
    assert.ok(ratio >= minimum && ratio <= maximum, `${viewport}px brand ratio must remain proportional`);
  }
...
test("pending intake request and status presentation remain authority-minimal", () => {
  ...
  assert.deepEqual(invited.access.actions, ["interrupt_intake", "cancel_intake"]);
  const started = pendingIntakePresentation(pendingIntake({ intake_status: "in_progress", effective_access: "INTERRUPTED" }));
  assert.equal(started.intake.label, "Intake gestart");
  assert.deepEqual(started.access.actions, ["resume_intake", "cancel_intake"]);
  assert.deepEqual(pendingIntakePresentation(pendingIntake({ effective_access: "EXPIRED" })).access.actions, ["reactivate_intake"]);
  assert.deepEqual(pendingIntakePresentation(pendingIntake({ effective_access: "CANCELLED" })).access.actions, []);
  assert.equal(pendingIntakePresentation(pendingIntake({ intake_status: "submitted" })), null);
...
test("Intake opvolging is a lazy dedicated workspace with guarded cleanup", async () => {
  ...
  ]) assert.match(pendingSection, new RegExp(`id="${id}"`));
  for (const action of ["interrupt_intake", "cancel_intake", "resume_intake", "reactivate_intake"]) {
    assert.match(pendingSection, new RegExp(`data-pending-lifecycle-action="${action}"`));
  }
