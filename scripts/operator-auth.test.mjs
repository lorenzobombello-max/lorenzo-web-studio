...
function createLoginHarness(authOverrides={},options={}) {
  ...
  const {rpc=async()=>({data:{display_name:"Current User",role:"owner",status:"ACTIVE"},error:null}),...controllerOptions}=options;
...
test("successful verification navigates to the operator profile",async()=>{let request;const harness=createLoginHarness({verifyOtp:async(value)=>(request=value,{data:{session},error:null})});await harness.controller.requestCode();harness.elements.codeInput.value="123456";assert.equal(await harness.controller.verifyCode(),true);assert.deepEqual(request,{email:"owner@example.com",token:"123456",type:"email"});assert.equal(harness.destination,OPERATOR_ROUTES.profile)});
test("parallel SIGNED_IN effects cannot produce duplicate OTP navigation",async()=>{const harness=createLoginHarness();await harness.controller.requestCode();harness.elements.codeInput.value="123456";const first=harness.controller.verifyCode();const second=harness.controller.verifyCode();assert.equal(await second,false);assert.equal(await first,true);assert.equal(harness.destination,OPERATOR_ROUTES.profile)});
...
test("active operator passes the current identity authority probe",async()=>assert.equal((await requireAuthorizedOperator({auth:{getSession:async()=>({data:{session},error:null})},rpc:async(name,parameters)=>(assert.equal(name,"get_current_operator_identity_v1"),assert.deepEqual(parameters,{}),{data:{display_name:"Current User",role:"owner",status:"ACTIVE"},error:null})},1700000000)).status,"authorized"));
test("non-operator session is denied by the current identity authority probe",async()=>assert.equal((await requireAuthorizedOperator({auth:{getSession:async()=>({data:{session},error:null})},rpc:async()=>({data:null,error:{code:"42501",message:"UNKNOWN_OPERATOR"}})},1700000000)).status,"unauthorized"));
