...
- [ ] **Step 4: Deploy Pages Functions without DNS.** Direct-upload the reviewed catch-all Function/config to the existing empty project `lws-website-project-preview-host`, configure the encrypted origin token, set fail-closed quota behavior, and prove the deployment at the assigned `lws-website-project-preview-host.pages.dev` hostname through negative origin/auth probes before making it discoverable to customers.
