...
- [x] **Step 1: Inventory production state read-only.** Captured in the local verification checkpoint. Production workspace authority is now confirmed through the Supabase Management API. GitHub Actions metadata is readable and proves the preview workflow/three required variables are absent. GitHub App installation details, Cloudflare account/plan/project/hostname, Supabase billing/spend cap and OVH management authority remain dashboard-only and unavailable to this session. Wrangler callback and device authorization did not produce a valid session. The target `pages.dev` hostname/CNAME remains explicitly unknown.
...
- [x] **Step 3: Validate local permission contracts read-only.** GitHub workflow jobs: fetch `id-token:write`, upload `id-token:write`, build no OIDC; broker request contract: one repository and `contents:read`; Supabase RPC grants: workflow functions service-role only, operator acquire/finalize/session functions authenticated only. Live GitHub App installation `161436785` repository selection and Contents permission are not confirmed: both installation API routes return `403` with the available credential.
...
- [ ] **Step 5: Obtain explicit approvals.** Cloudflare Workers Paid is confirmed active and the prior hosting/CNAME cost approval remains valid; OVH DNS edit/rollback authority is confirmed. Wrangler OAuth remains unconfirmed. Production migration/deploy and controlled 0006 dispatch remain separately gated. Before any GitHub App change, inspect installation `161436785` read-only at `https://github.com/organizations/lorenzo-web-solutions/settings/installations/161436785` and record repository selection plus Contents permission. The earlier claim that the customer repository was absent is withdrawn; do not create a second repository.
