...
- [x] **Step 5: Associate the custom subdomain and add one OVH CNAME.** Existing Pages association from clean `e59f3e4393932f7cf27b7671cff793bbd305d622` is now `active/active`; Lorenzo added exactly `preview CNAME lws-website-project-preview-host.pages.dev.` in OVH. Both authoritative servers and two public resolvers return that target with TTL 3600. TLS 1.3 has a valid hostname chain through `2026-12-23T04:44:20Z`. Custom host without session returns `401 PREVIEW_SESSION_REQUIRED` with `private, no-store`; production/immutable pages.dev remain 308 and direct origin 403. Only afterward was the exact host URL binding set and digest-verified; operator runtime auto-refreshed to active version 137 with unchanged bundle digest. Positive handoff/replay, cookie and asset navigation remain integral Gate-5 acceptance evidence. No other DNS, project-create, workflow, dispatch, push or merge occurred.
- [ ] **Step 6: Merge/publish the reviewed workflow commit.** Blocked before mutation on 2026-09-24: platform repo ID `1320223175` remote `main` is `ededdd6043c3ad749a1faf9a3993221c24fc3971` and does not contain `.github/workflows/build-website-project-preview.yml`; the reviewed local workflow was introduced at `35fbb65730966430d16432f8dea8ded8f655f474` and passes 3/3 manual-only/isolation tests. There are zero remote previewworkflowruns and all three preview Actions-variables remain absent. Publishing requires the explicitly excluded push/merge, so no variable write, AAL2 acquire or dispatch occurred.
