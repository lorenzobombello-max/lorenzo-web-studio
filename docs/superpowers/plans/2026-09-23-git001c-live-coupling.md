...
- [ ] **Step 2: Apply additive migrations in timestamp order. BLOCKED.** Obtain a new completed recoverypoint or explicitly approved equivalent recovery coverage for the post-backup auth changes, then repeat preflight and changes-since-backup review. Only after acceptance apply through `20260923120000`; verify columns, triggers, RPC definitions and grants before function deployment. Use `scripts/invoke-git001c-preview-release.ps1 -Phase ApplyMigrations -ExpectedHead <reviewed-sha> -Execute`; do not mark any migration manually and do not reuse historical backup `1759100262` without fresh provider verification.
...
- [ ] **Step 4: Deploy Pages Functions without DNS.** Direct-upload the reviewed catch-all Function/config to the existing empty project `lws-website-project-preview-host` with `wrangler pages deploy ./public --project-name lws-website-project-preview-host --branch main`; configure the encrypted origin token with `wrangler pages secret put`, set fail-closed quota behavior, and prove the deployment at `lws-website-project-preview-host.pages.dev` through negative origin/auth probes before making it discoverable to customers. Never rerun project-create.
