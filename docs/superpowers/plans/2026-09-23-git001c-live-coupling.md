...
- [ ] **Step 2: Apply additive migrations in timestamp order.** Apply through `20260923120000`; verify columns, triggers, RPC definitions and grants before function deployment. Use `scripts/invoke-git001c-preview-release.ps1 -Phase ApplyMigrations -ExpectedHead <reviewed-sha> -Execute`; do not mark any migration manually.
