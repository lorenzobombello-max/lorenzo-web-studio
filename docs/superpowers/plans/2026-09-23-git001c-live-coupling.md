...
- [x] **Step 3: Validate local permission contracts read-only.** GitHub workflow jobs: fetch `id-token:write`, upload `id-token:write`, build no OIDC; broker request contract: server-resolved repository ID `1378797607` only and `contents:read`; Supabase RPC grants: workflow functions service-role only, operator acquire/finalize/session functions authenticated only. Fresh targeted evidence is 6/6 source-token tests plus 1/1 exact-repository-scope and 1/1 project-file-read broker tests. Installation permissions and the specific previewtoken scope are separate layers; effective live token scope remains a post-deployment verification.
