...
- [ ] **Step 5: Obtain explicit approvals.** Cloudflare Workers Paid is confirmed active and the prior hosting/CNAME cost approval remains valid; OVH DNS edit/rollback authority is confirmed. Production migration/deploy and controlled 0006 dispatch remain separately gated. GitHub App installation `161436785` currently does not select the customer repository and is a hard stop until corrected and re-verified read-only; do not create a second repository.
