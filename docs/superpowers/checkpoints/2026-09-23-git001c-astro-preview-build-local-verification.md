...
| GitHub | Repositorynaam, immutable ID/node en exact verified commit zijn via REST bevestigd. De beschikbare repositorycredential kan Actions variables, secret-namen en environments lezen. De eigenaar heeft dashboard-side bevestigd dat GitHub App-installatie `161436785` de klantrepository **niet heeft geselecteerd**. | De ontbrekende repositoryselectie is een werkelijke authorityblocker voor de één-repository source-tokenbroker. De actuele `contents:read`-permission is niet afzonderlijk bevestigd. Daarnaast ontbreken de drie vereiste repositoryvariables `LWS_PREVIEW_SOURCE_TOKEN_ENDPOINT`, `LWS_PREVIEW_ARTIFACT_ENDPOINT` en `LWS_PREVIEW_OIDC_AUDIENCE` werkelijk; de previewworkflow staat nog niet op `main` (`404`). Productie bevat wel de bestaande App ID/installatie-ID/private-key-secretnamen. Geen dispatch. |
| Cloudflare | De eigenaar bevestigt dashboard-side dat **Cloudflare Workers Paid actief** is, met verlengdatum 2026-10-23. Wrangler 4.137.0 is beschikbaar via `npx`; de oude OAuth-credential is naar Windows Credential Manager gemigreerd maar niet meer geldig. Zowel localhost callback-login als twee officiële device-authorizationpogingen leverden geen nieuwe geldige sessie. | Pages-projecten, encrypted origin-secret, quota/fail-closed-instellingen en toegewezen `<project>.pages.dev` zijn niet bevestigd. De exacte CNAME-target is **onbekend**; geen placeholder publiceren. Het bestaande hostingakkoord blijft geldig en is niet opnieuw gevraagd. |
...
**Werkelijk ontbrekend:** de klantrepositoryselectie in GitHub App-installatie `161436785`; zeven nieuwe Supabase previewbindings; drie GitHub Actions previewvariables; de previewworkflow op `main`; alle zeven GIT-001C-productiemigraties; de drie previewfunctiedeployments.

**Niet bevestigd met de beschikbare toegang:** actuele GitHub App `contents:read`-permission; Cloudflare Pages-project/configuratie en toegewezen hostname. Er is dus nog geen onderbouwd CNAME-doel. De gewone browserlogin van de eigenaar wordt niet gedeeld met Wrangler of de geïntegreerde browser; er zijn geen wachtwoorden, cookies of tokens gevraagd.

Preflightbesluit: **NO-GO voor productie-uitvoering** zolang GitHub App-installatie `161436785` de klantrepository niet selecteert. Daarna moeten de App-permission en Cloudflare Pages-status nog read-only worden bevestigd en mogen de werkelijk ontbrekende bindings uitsluitend in een afzonderlijk goedgekeurde configuratie-/deploymentstap worden gezet. Deze NO-GO wijzigt het bestaande hostingakkoord niet.

Eerstvolgende uitvoerbare releaseactie, uitsluitend na een afzonderlijke expliciete configuratiegoedkeuring: wijzig GitHub App-installatie `161436785` zodat precies de bestaande klantrepository `lorenzo-web-solutions/lws-web-a88b1e8792714ad199ccb385b7982a8b` (ID `1378797607`) geselecteerd is; maak niets nieuws aan. Verifieer daarna read-only dat de selectie exact is en `contents:read` niet breder. Pas vervolgens, na de aparte production migration/deploygoedkeuring, controleer een verse backup en pas uitsluitend migratie `20260922180000_extend_website_project_preview_build_async_contract_v1.sql` toe. Stop daarna voor schema/trigger/grantvalidatie voordat `20260923060000` wordt toegepast. Workflowpublicatie, functiondeployments, Pages, DNS en dispatch blijven latere afzonderlijke gates.
