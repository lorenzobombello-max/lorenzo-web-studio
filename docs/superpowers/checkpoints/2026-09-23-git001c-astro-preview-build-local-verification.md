...
### Gate 4 custom domain, TLS en hostconfiguratie 2026-09-24
...
- OVH API-credentials ontbraken lokaal; Lorenzo heeft daarom handmatig exact één record toegevoegd: `preview CNAME lws-website-project-preview-host.pages.dev.` met standaard TTL. Beide authoritative OVH-servers geven exact dit doel met TTL 3600; Cloudflare DNS en Google Public DNS geven eveneens status 0 en exact één gelijk CNAME-antwoord. Er is geen tweede/conflicterend previewrecord gemaakt en de eerder vastgelegde nameservers, apex, `www`, MX en TXT zijn niet gewijzigd.
- De bestaande Cloudflare-associatie is niet opnieuw aangemaakt en rapporteert nu domainstatus `active` en verificationstatus `active`. HTTPS onderhandelt TLS 1.3; de geldige chain heeft subject/SAN `preview.lorenzowebsolutions.be`, issuer Google Trust Services `WE1`, geldigheid `2026-09-24T03:44:22Z` tot `2026-12-23T04:44:20Z`.
- `https://preview.lorenzowebsolutions.be/` bereikt aantoonbaar Pages plus de purpose-authenticated origin en geeft zonder previewsessie `401 PREVIEW_SESSION_REQUIRED` met `private, no-store`. Zowel de production als immutable pages.dev-host blijft `308` naar het custom hostpad sturen; rechtstreekse Supabase-origin blijft `403 PREVIEW_ORIGIN_FORBIDDEN`.
...
### Gate 5 runs en lokale storage-prefixcorrectie 2026-09-24

- PR `#55` publiceerde de manual-only workflow. De eerste afzonderlijk geautoriseerde run `35960635554` faalde in de geïsoleerde build met `EACCES` op `/workspace/node_modules`; zonder retry werd de authority opgeruimd. PR `#57` publiceerde daarna de dynamische non-root UID/GID-correctie en bracht `main` op `6e8a746385934a178a6453a43ea44b4861856e04`.
- Een nieuwe volledige preflight bevestigde de drie exacte Actions-variabelen, nul conflicterende authority en de ongewijzigde dossier-0006-binding aan repository ID `1378797607`, node `R_kgDOUi7IJw`, revision 1, `REPOSITORY_READY/BOUND` en commit `1f19bf01c61c6da79fa4c7374333a91b70f9bf48`. Eén owner-AAL2 acquire leverde lease `4b6f635f-2b05-45fd-a7ed-cdab443f6bda` en authorized build `aa685956-91a6-4891-af75-e11c50560bc7`; exact één dispatch gaf HTTP 204 en run `35965885425`, nummer 2/attempt 1.
- Fetch, geïsoleerde build, dist-publicatie en upload/finalisatie waren groen. Preview build `b93a0deb-48f3-4148-b39b-a39fab94b6ea` eindigde `PASS_WITH_WARNINGS` met zes manifestartifacts en primair `index.html`. De eenmalige handoff werd eenmaal geconsumeerd, maar de sessiegebonden root gaf 404 `PREVIEW_ASSET_NOT_FOUND` met `private, no-store`.
- De bewezen oorzaak is een server-side ID-contractmismatch: upload schrijft vóór finalisatie naar `{authorized_build_id}/{relative_path}` en alle zes objecten stonden onder `aa685956-91a6-4891-af75-e11c50560bc7/`; finalisatie creëert een afzonderlijk `preview_build_id`, waarna de host ten onrechte onder het lege `b93a0deb-48f3-4148-b39b-a39fab94b6ea/` las. Er is geen Storage-object verplaatst.
- Fail-closed cleanup liet de vrijgegeven lease, `FINALIZED` upload, completed build, manifest en objecten intact. Alleen viewersessie `ba609d11-c855-406d-b9b6-0f66a874612f` is gerevoked; stale-cookie access gaf daarna 401 `PREVIEW_SESSION_INVALID`. Finale readback: nul actieve leases, artifacttokens, open uploads en viewersessies. Geen retry, tweede acquire/dispatch, providercreate, DNS- of klantrepositorymutatie; `NO_SECOND_CREATE` bleef HARD.
- De lokale correctie is gebouwd in worktree `lorenzo-web-studio-git001c-storage-prefix-fix-20260924` vanaf exact `origin/main` `6e8a746385934a178a6453a43ea44b4861856e04`. `authorized_build_id` is het enige leidende Storage-prefix, omdat dit vóór upload server-side op de lease is vastgelegd en door artifacttoken/uploadsessie-bindings wordt afgedwongen. Een additieve resolvermigratie geeft dit ID terug via session -> preview build -> lease, zonder released leases uit te sluiten; de host accepteert alleen een geldig UUID en heeft geen fallback naar `previewBuildId`.
