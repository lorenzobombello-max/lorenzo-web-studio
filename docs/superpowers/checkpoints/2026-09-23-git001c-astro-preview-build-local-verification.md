...
| GitHub | Repositorynaam, immutable ID/node en exact verified commit zijn via REST bevestigd. De beschikbare repositorycredential kan Actions variables, secret-namen en environments lezen. De eerdere vragenlijstselectie dat de klantrepository niet geselecteerd zou zijn, was onjuist en is ingetrokken. Read-only API-requests voor user-installations en installatie `161436785` geven met de beschikbare credential beide `403`. | Repositoryselectie van installatie `161436785` en de actuele Contents-permissies zijn **niet bevestigd**; er is niet vastgesteld dat repository ID `1378797607` aanwezig of afwezig is. Daarnaast ontbreken de drie vereiste repositoryvariables `LWS_PREVIEW_SOURCE_TOKEN_ENDPOINT`, `LWS_PREVIEW_ARTIFACT_ENDPOINT` en `LWS_PREVIEW_OIDC_AUDIENCE` werkelijk; de previewworkflow staat nog niet op `main` (`404`). Productie bevat wel de bestaande App ID/installatie-ID/private-key-secretnamen. Geen dispatch. |
| Cloudflare | De eigenaar bevestigt dashboard-side dat **Cloudflare Workers Paid actief** is, met verlengdatum 2026-10-23. Wrangler 4.137.0 is beschikbaar via `npx`; de oude OAuth-credential is naar Windows Credential Manager gemigreerd maar niet meer geldig. Zowel localhost callback-login als twee officiële device-authorizationpogingen leverden geen nieuwe geldige sessie. Wrangler OAuth-autorisatie is dus **niet succesvol afgerond**. | Pages-projecten, encrypted origin-secret, quota/fail-closed-instellingen en toegewezen `<project>.pages.dev` zijn niet bevestigd. De exacte CNAME-target is **onbekend**; geen placeholder publiceren. Het bestaande hostingakkoord blijft geldig en is niet opnieuw gevraagd. |
...
**Bevestigd correct:** production workspacebinding en exacte klantrepository/commit; Supabase projectgezondheid, databaseversie, dagelijkse backup, Pro Plan, ingeschakelde spend cap en bestaande private bucket; Cloudflare Workers Paid; bestaande GitHub App-secretnamen; OVH DNS-zonebekijk-/bewerkbevoegdheid; publieke OVH-nameservers en ongewijzigde mailroute.

**Werkelijk ontbrekend:** zeven nieuwe Supabase previewbindings; drie GitHub Actions previewvariables; de previewworkflow op `main`; alle zeven GIT-001C-productiemigraties; de drie previewfunctiedeployments. Repositoryselectie en Contents-permissies van GitHub App-installatie `161436785` vallen nadrukkelijk niet in deze categorie.

**Niet bevestigd met de beschikbare toegang:** of GitHub App-installatie `161436785` bestaande klantrepository ID `1378797607` selecteert en welke actuele Contents-permissies gelden; Cloudflare Pages-project/configuratie en toegewezen hostname; Wrangler OAuth-autorisatie. Er is dus nog geen onderbouwd CNAME-doel. De gewone browserlogin van de eigenaar wordt niet gedeeld met Wrangler of de geïntegreerde browser; er zijn geen wachtwoorden, cookies of tokens gevraagd.

Preflightbesluit: **NO-GO voor productie-uitvoering** totdat repositoryselectie en Contents-permissies van GitHub App-installatie `161436785` read-only zijn bevestigd. Er wordt niet aangenomen dat de klantrepository ontbreekt en de installatie wordt op basis van de ingetrokken vragenlijstselectie niet gewijzigd. Daarna moeten de Cloudflare Pages-status en werkelijk ontbrekende bindings uitsluitend binnen hun afzonderlijk goedgekeurde configuratie-/deploymentstap worden behandeld. Deze NO-GO wijzigt het bestaande hostingakkoord niet.

Eerstvolgende concrete vervolgstap is uitsluitend read-only: open `https://github.com/organizations/lorenzo-web-solutions/settings/installations/161436785` in de aangemelde gewone browser en bekijk zonder opslaan of wijzigen onder **Repository access** of `lws-web-a88b1e8792714ad199ccb385b7982a8b` (ID `1378797607`) geselecteerd is, plus onder **Permissions** welke toegang voor **Contents** geldt. Leg alleen `All repositories` of `Only select repositories` plus aanwezigheid/afwezigheid van de exacte repository en `Contents: Read-only`/afwijking vast. Pas na dat bewijs kan een eventuele configuratieactie afzonderlijk worden beoordeeld. Workflowpublicatie, migraties, functiondeployments, Pages, DNS en dispatch blijven latere afzonderlijke gates.
