...
const standaloneInitializers = new Map([
  ["dossiers", async ({ root, client, identity, onInvalidate, onAuthorizationFailure, requireAal2, requestOpen, slotKey })=>{
    if (String(slotKey || "").startsWith("req-")) {
      const { initializeOperatorProjectRequirements } = await import("./operator-project-requirements-child.mjs?v=20260910-requirements-sync-v1");
    ...
    if (String(slotKey || "").startsWith("website-")) {
      const { initializeOperatorWebsiteExecution } = await import("./operator-website-execution-child.mjs?v=20260910-requirements-sync-v1");
    ...
    if (String(slotKey || "").startsWith("project-")) {
      const { initializeOperatorProjectWorkspace } = await import("./operator-project-workspace-child.mjs?v=20260910-requirements-sync-v1");
  ...
  ["calendar", async ({ root, client, identity, onAuthorizationFailure })=>{
    const { initializeOperatorCalendar } = await import("./operator-calendar.mjs?v=20260905-calendar-selection-r5");
