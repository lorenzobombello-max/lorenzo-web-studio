...
select ok(
  ...
  'pending-intake list is executable only through authenticated caller authority'
);

select ok(
  pg_get_functiondef('public.list_operator_pending_intakes_v1(uuid,text)'::regprocedure)
    like '%auth.uid() <> p_actor_auth_user_id%',
  'pending-intake list binds the requested operator UUID to auth.uid()'
);
...
select lives_ok(
  $$select public.list_operator_pending_intakes_v1('c9bcd3ef-1e7e-4889-8a12-db827f1b97b0', 'ACTIVE')$$,
  'authorized OP-01 caller can read pending intakes'
);
select throws_ok(
  $$select public.list_operator_pending_intakes_v1('bd2ab636-0d42-4069-88a9-60bd97f2b335', 'ACTIVE')$$,
...
set local role authenticated;
select throws_ok(
  $$select public.list_operator_pending_intakes_v1('d0247fd9-60d5-40bc-a905-6b02024b6420', 'ACTIVE')$$,
  '42501', 'APPLICATION_SCOPE_DENIED',
