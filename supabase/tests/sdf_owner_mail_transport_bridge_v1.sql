...
select has_function(
  ...
  array['uuid','uuid','integer','text','uuid','uuid','text','bigint'],
  'QF-5I owner-authorized SDF mail transport context bridge exists'
);
...
select is(
  pg_get_function_arguments(
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)'::regprocedure
  ),
  'p_business_draft_id uuid, p_approval_id uuid, p_expected_approval_version integer, p_expected_approval_sha256 text, p_issuance_id uuid, p_artifact_id uuid, p_expected_artifact_sha256 text, p_expected_artifact_bytes bigint',
  'transport bridge accepts only frozen lineage and artifact identity'
);
select ok(
  has_function_privilege(
    'authenticated',
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)',
    'execute'
  ) and not has_function_privilege(
    'anon',
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)',
  ...
  'authenticated is the only browser role allowed to read transport context'
);
select ok(
  not has_function_privilege(
    'service_role',
  ...
  'service role cannot bypass SDF owner business authority'
);
select ok(
  pg_get_functiondef(
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)'::regprocedure
  ...
  'bridge is owner-authorized and contains no admin token, claim, mutation, transport, or dynamic SQL'
);

create temporary table qf5i_baseline as select
  ...
  (select count(*) from public.quote_request_email_jobs) email_job_count;
create temporary table qf5i_context as
select context.* from qf5b_fixture fixture
cross join lateral public.get_sdf_quotation_delivery_transport_context_v1(
  fixture.business_draft_id,fixture.approval_id,1,fixture.approval_sha256,
  fixture.issuance_id,fixture.artifact_id,repeat('9',64),100
) context;

select is((select issuance_id from qf5i_context),(select issuance_id from qf5b_fixture),
  'owner transport context returns the exact prepared issuance');
select is((select capability_id from qf5i_context),(select capability_id from qf5b_delivery),
  'owner transport context returns the existing ACTIVE capability');
select is((select orchestration_id from qf5i_context),(select orchestration_id from qf5b_delivery),
  'owner transport context returns the existing immutable orchestration');
select is((select email_job_id from qf5i_context),(select email_job_id from qf5b_delivery),
  'owner transport context returns the existing delivery job');
select is((select recipient_email from qf5i_context),'qf5b@example.test',
  'recipient comes from immutable prepared orchestration authority');
select is((select content_version from qf5i_context),'QUOTATION_DELIVERY_NL_BE_v1',
  'template comes from immutable prepared orchestration authority');
select ok(
  (select stored_token_digest=repeat('a',64)
    and encrypted_token='v1.AAAAAAAAAAAAAAAA.AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'
    from qf5i_context),
  'only existing encrypted token material and digest are returned'
);
select ok(
  (select artifact_id=(select artifact_id from qf5b_fixture)
    and artifact_sha256=repeat('9',64) and artifact_bytes=100
    from qf5i_context),
  'transport context remains bound to the exact registered DOCX authority'
);
select ok(
  (select job_status='pending' and attempt_count=0 and sent_at is null
    from qf5i_context),
  'bridge returns an unclaimed pending job without transport state changes'
);

select set_config('request.jwt.claim.sub',pg_temp.qf5b_uuid('qf5b-operator-auth')::text,true);
select throws_ok(format(
  $sql$select * from public.get_sdf_quotation_delivery_transport_context_v1(%L,%L,1,%L,%L,%L,%L,100)$sql$,
  (select business_draft_id from qf5b_fixture),(select approval_id from qf5b_fixture),
  (select approval_sha256 from qf5b_fixture),(select issuance_id from qf5b_fixture),
  (select artifact_id from qf5b_fixture),repeat('9',64)
),'42501','OWNER_REQUIRED','authenticated non-owner cannot read transport context');
select set_config('request.jwt.claim.sub',pg_temp.qf5b_uuid('qf5b-owner-auth')::text,true);
select throws_ok(format(
  $sql$select * from public.get_sdf_quotation_delivery_transport_context_v1(%L,%L,1,%L,%L,%L,%L,100)$sql$,
  (select business_draft_id from qf5b_fixture),(select approval_id from qf5b_fixture),
  (select approval_sha256 from qf5b_fixture),pg_temp.qf5b_uuid('qf5i-wrong-issuance'),
  (select artifact_id from qf5b_fixture),repeat('9',64)
),'P0001','SDF_DELIVERY_TRANSPORT_NOT_AVAILABLE','wrong issuance is rejected');
select throws_ok(format(
  $sql$select * from public.get_sdf_quotation_delivery_transport_context_v1(%L,%L,1,%L,%L,%L,%L,100)$sql$,
  (select business_draft_id from qf5b_fixture),(select approval_id from qf5b_fixture),
  (select approval_sha256 from qf5b_fixture),(select issuance_id from qf5b_fixture),
  pg_temp.qf5b_uuid('qf5i-wrong-artifact'),repeat('9',64)
),'P0001','SDF_DELIVERY_TRANSPORT_NOT_AVAILABLE','wrong artifact is rejected');

update public.quote_request_email_jobs set encrypted_payload=null
where id=(select email_job_id from qf5b_delivery);
select throws_ok(format(
  $sql$select * from public.get_sdf_quotation_delivery_transport_context_v1(%L,%L,1,%L,%L,%L,%L,100)$sql$,
  (select business_draft_id from qf5b_fixture),(select approval_id from qf5b_fixture),
  (select approval_sha256 from qf5b_fixture),(select issuance_id from qf5b_fixture),
  (select artifact_id from qf5b_fixture),repeat('9',64)
),'P0001','SDF_DELIVERY_PAYLOAD_UNAVAILABLE','missing prepared encrypted payload is rejected');
update public.quote_request_email_jobs
set encrypted_payload='v1.AAAAAAAAAAAAAAAA.AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'
where id=(select email_job_id from qf5b_delivery);

select ok(
  pg_get_functiondef(
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)'::regprocedure
  ) like '%v_capability.status <> ''ACTIVE''%'
  and pg_get_functiondef(
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)'::regprocedure
  ) like '%v_capability.expires_at <= clock_timestamp()%'
  and pg_get_functiondef(
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)'::regprocedure
  ) like '%quote_request_quotation_acceptances%'
  and pg_get_functiondef(
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)'::regprocedure
  ) like '%is_quotation_within_validity_v1%',
  'bridge rejects inactive or expired capability, accepted quotation, and expired validity'
);
select ok(
  pg_get_functiondef(
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)'::regprocedure
  ) like '%v_orchestration.email_type <> ''QUOTATION_DELIVERY''%'
  and pg_get_functiondef(
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)'::regprocedure
  ) like '%v_orchestration.issuance_id is distinct from p_issuance_id%'
  and pg_get_functiondef(
    'public.get_sdf_quotation_delivery_transport_context_v1(uuid,uuid,integer,text,uuid,uuid,text,bigint)'::regprocedure
  ) like '%v_job.kind <> ''quotation_delivery''%',
  'bridge requires the exact prepared orchestration and quotation delivery job binding'
);

create temporary table qf5i_replay as
select context.* from qf5b_fixture fixture
cross join lateral public.get_sdf_quotation_delivery_transport_context_v1(
  fixture.business_draft_id,fixture.approval_id,1,fixture.approval_sha256,
  fixture.issuance_id,fixture.artifact_id,repeat('9',64),100
) context;
select is((select row(qf5i_replay.*)::text from qf5i_replay),
  (select row(qf5i_context.*)::text from qf5i_context),
  'replay returns the same immutable transport context');
select ok(
  (select count(*)=(select capability_count from qf5i_baseline)
    from public.quote_request_quotation_acceptance_capabilities)
  and (select count(*)=(select orchestration_count from qf5i_baseline)
    from public.quote_request_quotation_email_orchestrations)
  and (select count(*)=(select email_job_count from qf5i_baseline)
    from public.quote_request_email_jobs),
  'bridge replay creates no capability, orchestration, or email job'
);
select ok(
