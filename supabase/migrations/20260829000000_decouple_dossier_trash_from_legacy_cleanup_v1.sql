create or replace function public.execute_operator_dossier_lifecycle_command_v1(
...
comment on function public.execute_operator_dossier_lifecycle_command_v1(uuid, text, bigint, uuid, text, uuid) is
  'Edge-capability-gated authenticated owner/admin dossier-zone command boundary with revision guards, idempotent audit events, trash independent from permanent-purge eligibility, restore-to-prior-state semantics, and no purge authority.';