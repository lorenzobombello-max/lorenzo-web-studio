alter table public.quote_request_pricing_snapshots
  add column snapshot_contract_version smallint;

alter table public.quote_request_pricing_snapshots
  add constraint quote_request_pricing_snapshots_contract_version_valid
    check (snapshot_contract_version is null or snapshot_contract_version = 2);
    ...
    check (
      ...
      and (
        ...
        or (
          ...
          and jsonb_typeof(budget_evaluation->'status') = 'string'
...
comment on column public.quote_request_pricing_snapshots.snapshot_contract_version is
  'Null identifies immutable historical v1 snapshots. Value 2 identifies the closed provenance-aware tri-state snapshot contract.';

create function public.update_quote_request_intake_v3(
...
begin
  ...
  if jsonb_typeof(v_budget_evaluation) is distinct from 'object'
     ...
     or v_budget_evaluation->>'evidenceProvenance' not in ('budget_guard_v1', 'legacy_label') then
...
comment on function public.update_quote_request_intake_v3(text, text, jsonb, text, timestamptz, jsonb) is
  'Service-role-only atomic submit boundary for closed pricing snapshot contract v2. It stores authoritative output but never calculates pricing.';
...
create function public.inspect_quote_request_intake_details_v3(p_access_token_hash text)
returns table (
  intake_id uuid,
  intake_status text,
...
as $$
  ...
  from public.inspect_quote_request_intake_details_v2(p_access_token_hash) as legacy
  left join public.quote_request_pricing_snapshots as snapshot
    on snapshot.intake_id = legacy.intake_id
$$;

revoke all
on function public.inspect_quote_request_intake_details_v3(text)
from public, anon, authenticated;
...
create function public.inspect_submitted_intake_for_admin_v3(p_admin_access_token_hash text)
returns table (
  intake_id uuid,
  intake_status text,
...
as $$
  ...
  from public.inspect_submitted_intake_for_admin_v2(p_admin_access_token_hash) as legacy
  left join public.quote_request_pricing_snapshots as snapshot
    on snapshot.intake_id = legacy.intake_id
$$;

revoke all
on function public.inspect_submitted_intake_for_admin_v3(text)
from public, anon, authenticated;
