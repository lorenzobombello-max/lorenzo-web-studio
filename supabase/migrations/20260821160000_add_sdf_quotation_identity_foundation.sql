create table public.sdf_quotations (
  quotation_id uuid primary key default gen_random_uuid(),
  quote_request_id uuid not null unique references public.quote_requests(id),
  created_at timestamptz not null default clock_timestamp()
);

comment on table public.sdf_quotations is
  'Private identity-only foundation for one future SDF quotation per SDF application. No status, pricing, issuance, delivery, acceptance, validity, or Website quotation semantics.';

create function public.guard_sdf_quotation_identity_v1()
returns trigger
language plpgsql
...
create trigger trg_sdf_quotation_identity_guard
before insert or update or delete on public.sdf_quotations
for each row execute function public.guard_sdf_quotation_identity_v1();

alter table public.sdf_quotations enable row level security;
alter table public.sdf_quotations force row level security;

revoke all privileges on table public.sdf_quotations
from public, anon, authenticated, service_role;
...
begin
  ...
  select jsonb_build_object(
    ...
    'sdf_quotation', case
      when qr.request_kind = 'slimme_documentenflow' and sdf_quotation.quotation_id is not null
        then jsonb_build_object(
          'quotation_id', sdf_quotation.quotation_id,
          'quote_request_id', qr.id,
          'application_reference', qr.application_reference,
          'created_at', sdf_quotation.created_at
        )
      else null
    end,
    'name', qr.name,
  ...
  left join public.sdf_quotations as sdf_quotation
    on sdf_quotation.quote_request_id = qr.id
   and qr.request_kind = 'slimme_documentenflow'
...
comment on function public.get_operator_application_v1(uuid, text) is
  'Owner/admin-only product-aware dossier with isolated Website evidence and private identity-only SDF quotation and project authorities. SDF quotation status, pricing duplication, issuance, acceptance, and lifecycle remain unavailable.';