...
comment on function public.list_operator_applications_v1(integer, integer) is
  'Owner/admin-only submitted application list with authoritative request kind. Caller identity is derived from auth.uid(); application_reference is a locator only.';
comment on function public.get_operator_application_v1(uuid, text) is
  'Owner/admin-only application dossier with authoritative request kind, pricing, quotation, acceptance and project linkage. Locators never authorize access.';