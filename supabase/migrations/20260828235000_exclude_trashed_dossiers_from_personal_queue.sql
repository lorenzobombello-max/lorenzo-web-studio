create or replace function public.get_operator_personal_dossier_queue_v1(
...
comment on function public.get_operator_personal_dossier_queue_v1(text, integer) is
  'Read-only current non-trashed dossier assignments for the authenticated ACTIVE operator; caller identity derives exclusively from auth.uid().';