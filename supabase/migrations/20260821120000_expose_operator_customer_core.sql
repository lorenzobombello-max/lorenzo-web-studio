...
comment on function public.get_operator_application_v1(uuid, text) is
  'Owner/admin-only product-aware dossier with persisted shared Customer Core. Website evidence remains intake-backed; Documentenflow exposes no Website artifacts.';