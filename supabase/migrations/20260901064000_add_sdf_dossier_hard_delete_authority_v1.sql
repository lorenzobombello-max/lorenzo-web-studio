create function lws_internal.sdf_dossier_purge_block_reason_v1(
...
as $$
declare
  v_has_optional_dependency boolean;
begin
  if exists (
    select 1 from public.sdf_quotations
    where quote_request_id = p_quote_request_id
  ) then
    return 'SDF_QUOTATION_EXISTS';
  end if;

  if exists (
    select 1 from public.sdf_quotation_preparation_authorities
    where quote_request_id = p_quote_request_id
  ) then
    return 'QUOTATION_PREPARATION_EXISTS';
  end if;

  if exists (
    select 1
    from public.sdf_quotation_acceptances as acceptance
    join public.sdf_quotation_documents as document
      on document.quotation_id = acceptance.quotation_id
    join public.sdf_quotations as quotation
      on quotation.quotation_id = document.quotation_id
  ...
  if exists (
    select 1 from public.sdf_m1_invoice_candidates
    where quote_request_id = p_quote_request_id
  ) or exists (
    select 1
  ...
  if exists (
    select 1 from public.sdf_projects
    where quote_request_id = p_quote_request_id
  ) or exists (
    select 1
  ...
  if to_regclass('public.sdf_owner_work_acceptance_authorities') is not null then
    execute
      'select exists (
         select 1 from public.sdf_owner_work_acceptance_authorities
         where quote_request_id = $1
       )'
      into v_has_optional_dependency
      using p_quote_request_id;
    if v_has_optional_dependency then
...
create function public.can_purge_sdf_dossier_v1(p_quote_request_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
create or replace function lws_internal.reject_sdf_qualification_history_mutation_v1()
returns trigger
language plpgsql
...
begin
  ...
  raise exception using errcode = '55000',
    message = 'SDF_QUALIFICATION_HISTORY_IMMUTABLE';
end;
$$;

create or replace function lws_internal.prevent_sdf_initial_confirmation_recovery_event_mutation_v1()
returns trigger
language plpgsql
set search_path = pg_catalog
as $$
...
revoke all on function public.can_purge_sdf_dossier_v1(uuid)
from public, anon, authenticated, service_role;
grant execute on function public.can_purge_sdf_dossier_v1(uuid) to authenticated;
