...
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    return jsonResponse(500, { ok: false, code: "SERVER_CONFIGURATION_ERROR", message: "Service unavailable." }, origin);
  }
  ...
  try {
    ...
    const requestError = error instanceof RequestError ? error : new RequestError(400, "INVALID_REQUEST");
    return jsonResponse(requestError.status, { ok: false, code: requestError.code, message: "Invalid request." }, origin);
  }
  ...
  try {
    ...
    const code = error instanceof PrivacyRequestValidationError ? error.code : "INVALID_INPUT";
    const field = error instanceof PrivacyRequestValidationError ? error.field : undefined;
    return jsonResponse(400, { ok: false, code, ...(field ? { field } : {}), message: "Invalid input." }, origin);
  }

  const idempotencyKey = (request.headers.get("idempotency-key") || "").trim();
  if (!IDEMPOTENCY_KEY_REGEX.test(idempotencyKey)) {
    return jsonResponse(400, { ok: false, code: "INVALID_IDEMPOTENCY_KEY", message: "Invalid request." }, origin);
  }
  ...
  const requestFingerprint = await fingerprintPayload(payload as unknown as Record<string, unknown>);
  ...
  if (existingRequest) {
    if (existingRequest.request_fingerprint !== requestFingerprint) {
      return jsonResponse(409, { ok: false, code: "IDEMPOTENCY_CONFLICT", message: "Could not process request." }, origin);
    }
    return jsonResponse(200, { ok: true, code: "REQUEST_RECEIVED", request_id: existingRequest.id }, origin);
  }
  ...
  try {
    ...
    return jsonResponse(500, { ok: false, code: "REQUEST_SECURITY_ERROR", message: "Could not process request." }, origin);
  }

  const configuredWindowSeconds = Number(Deno.env.get("PRIVACY_RATE_LIMIT_WINDOW_SECONDS") || "900");
  const configuredMaxRequests = Number(Deno.env.get("PRIVACY_RATE_LIMIT_MAX_REQUESTS") || "3");
  const windowSeconds = Number.isFinite(configuredWindowSeconds) && configuredWindowSeconds > 0
    ? configuredWindowSeconds
    : 900;
  const maxRequests = Number.isFinite(configuredMaxRequests) && configuredMaxRequests > 0
  ...
  if (rateLimitError) {
    console.error("privacy_rate_limit_error", { code: rateLimitError.code ?? "UNKNOWN" });
    return jsonResponse(503, { ok: false, code: "RATE_LIMIT_UNAVAILABLE", message: "Service temporarily unavailable." }, origin);
  }
  if ((count ?? 0) >= maxRequests) {
    return jsonResponse(429, { ok: false, code: "RATE_LIMITED", message: "Too many requests." }, origin);
  }

  const userAgent = (request.headers.get("user-agent") || "").slice(0, 500);
  const { data, error: createError } = await supabase.rpc("create_privacy_request_idempotent", {
  ...
  if (!created.was_created || created.request_notification_status === "sent") {
    return jsonResponse(200, { ok: true, code: "REQUEST_RECEIVED", request_id: created.request_id }, origin);
  }
  ...
  if (resendApiKey && adminEmail && fromEmail) {
    ...
    try {
      ...
      notificationErrorCode = error instanceof Error && error.name === "AbortError"
        ? "EMAIL_TIMEOUT"
  ...
  if (notificationUpdateError) {
    console.error("privacy_notification_status_update_error", { code: notificationUpdateError.code ?? "UNKNOWN" });
  }

  return jsonResponse(202, {
    ok: true,
    code: notificationStatus === "sent" ? "REQUEST_RECEIVED" : "REQUEST_STORED_NOTIFICATION_FAILED",
