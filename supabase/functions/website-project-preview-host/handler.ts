...
export type WebsiteProjectPreviewHostService = Readonly<{
  ...
  consumeHandoff(input: Readonly<{
    handoffTokenHash: string;
    viewerSessionTokenHash: string;
  }>): PromiseLike<Readonly<{ expiresAt: string }>>;
  resolveSession(viewerSessionTokenHash: string): PromiseLike<Readonly<{
...
function text(status: number, code: string): Response {
  return new Response(code, { status, headers: { "cache-control": "private, no-store" } });
}
...
function cookieToken(request: Request): string {
  ...
  return /(?:^|;\s*)lws_preview_session=([A-Za-z0-9_-]{32,256})(?:;|$)/.exec(cookies)?.[1] ?? "";
}
...
export async function handleWebsiteProjectPreviewHost(
  ...
  if (request.headers.get("x-lws-preview-origin-token") !== service.originToken || !service.originToken) {
  ...
  let url: URL;
  try {
    url = new URL(forwardedPath || new URL(request.url).pathname, "https://preview.lorenzowebsolutions.be");
  } catch {
  ...
  if (url.pathname === "/handoff") {
    ...
    return new Response(null, {
      ...
      headers: {
        ...
        "set-cookie": `${COOKIE_NAME}=${viewerToken}; HttpOnly; Secure; SameSite=Lax; Path=/`,
      },
    });
  }
  ...
  let session: Awaited<ReturnType<WebsiteProjectPreviewHostService["resolveSession"]>>;
  try {
  ...
  const artifact = session.artifacts.find((candidate) => candidate.relativePath === relativePath);
  if (!artifact) return text(404, "PREVIEW_ASSET_NOT_FOUND");
  try {
    const bytes = await service.download(`${session.previewBuildId}/${artifact.relativePath}`);
