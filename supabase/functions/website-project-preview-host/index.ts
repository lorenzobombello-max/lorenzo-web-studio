...
export function createWebsiteProjectPreviewHostRuntime(
  ...
  return Object.freeze({
    ...
    consumeHandoff: (input: Readonly<{ handoffTokenHash: string; viewerSessionTokenHash: string }>) =>
      rpc<Readonly<{ expiresAt: string }>>("consume_website_project_preview_handoff_v1", {
        p_handoff_token_hash: input.handoffTokenHash,
        p_viewer_session_token_hash: input.viewerSessionTokenHash,
      }),
    resolveSession: (viewerSessionTokenHash: string) => rpc<Readonly<{
    ...
    download: async (objectPath: string) => {
      ...
      if (error || !data) throw new Error(error?.message || "PREVIEW_ASSET_NOT_FOUND");
