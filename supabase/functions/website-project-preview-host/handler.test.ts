...
const previewBuildId = "22222222-2222-4222-8222-222222222222";
const storageBuildId = "33333333-3333-4333-8333-333333333333";
const otherBuildId = "44444444-4444-4444-8444-444444444444";
  ...
  return {
    ...
    download: async (path: string) =>
      new TextEncoder().encode(
        path.endsWith("index.html") ? "<h1>Preview</h1>" : "body",
      ),
    ...overrides,
  };
}
...
Deno.test("preview origin rejects unauthenticated direct requests before dependencies", async () => {
  let called = false;
  const response = await handleWebsiteProjectPreviewHost(
    new Request("https://origin.local/"),
    service({
      resolveSession: async () => {
        called = true;
...
Deno.test("preview origin atomically rotates one handoff into a host-only viewer cookie", async () => {
  let consumed:
    ...
    | undefined;
  const response = await handleWebsiteProjectPreviewHost(
    originRequest("/handoff?token=handoff-token"),
    service({
      consumeHandoff: async (
        input: { handoffTokenHash: string; viewerSessionTokenHash: string },
      ) => {
        consumed = input;
  ...
  assertEquals(
    consumed.handoffTokenHash === consumed.viewerSessionTokenHash,
    false,
  );
});

Deno.test("preview origin uses only the session-bound storage build for manifest assets", async () => {
  const downloads: string[] = [];
  const bound = service({
    download: async (path: string) => {
      downloads.push(path);
      return new TextEncoder().encode("<h1>Preview</h1>");
    },
  });
  for (const path of ["/", "/assets/styles.css"]) {
    const response = await handleWebsiteProjectPreviewHost(
      originRequest(path, {
  ...
  assertEquals(downloads, [
    `${storageBuildId}/index.html`,
    `${storageBuildId}/assets/styles.css`,
  ]);

  const absent = await handleWebsiteProjectPreviewHost(
    originRequest(`/${otherBuildId}/index.html`, {
      headers: { cookie: `lws_preview_session=${viewerToken}` },
    }),
    bound,
  );
...
Deno.test("preview origin rejects absent or expired viewer sessions", async () => {
  ...
  const expired = await handleWebsiteProjectPreviewHost(
    originRequest("/", {
      headers: { cookie: `lws_preview_session=${"e".repeat(64)}` },
    }),
    service({
      resolveSession: async () => {
        throw new Error("PROJECT_PREVIEW_SESSION_INVALID");
      },
    }),
  );
...
Deno.test("preview origin rejects missing or invalid session-bound storage build IDs", async () => {
  for (const invalidStorageBuildId of [undefined, "", "not-a-uuid"]) {
    ...
    const response = await handleWebsiteProjectPreviewHost(
      ...
      service({
        resolveSession: async () => ({
          previewBuildId,
          storageBuildId: invalidStorageBuildId,
          artifacts: [
            {
              relativePath: "index.html",
              contentType: "text/html; charset=utf-8",
    ...
    assertEquals(await response.text(), "PREVIEW_SESSION_INVALID");
